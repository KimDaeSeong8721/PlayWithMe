	...
	function chkFinish()
	{
		isFinish = board1.isFinish();
		if(isFinish)
		{
			alert('축하합니다. 스도쿠를 모두 풀었습니다! \n 경험치 30exp을 획득하셧습니다.');
			
			//window.location.href = '/term/jsp/countSudoku.jsp';
		}
	} 
	function test()
	{
	...
	function callHint() {
		...
		if(hintCount>0)
		{
			...
			//console.log(cell.getAnswer(),cell.getValue());
			if(cell.getAnswer()!=cell.getValue())
			{
			hintCount--;
			pushBoard(); //되돌리기 기록 
			//console.log('different');
			cell.setValue(cell.getAnswer());
			board1.deleteMemoAll(cell.getAnswer(),new Location(selectRow, selectCol));
	...
	function drawGrid() {  // 실제로 그려지는 그리드 영역 
		...
		for (var i = 0; i <= BoardSize; i++) {
			...
			if (!thick)
			{
				...
				context.strokeStyle = '#b8b0e0';  //구분선은 검정, 일반선은 bec6d4
		...
		for (var i = 0; i <= BoardSize; i++) {
			...
			{
				context.beginPath();  //경로를 시작합니다
				context.strokeStyle = '#887cc5' ;  //구분선은 검정, 일반선은 bec6d4
				context.lineWidth = 1.7 ;  //구분선은 두껍게 
				// 수직선을 그림 
	...
	function drawCells() {
		...
		if (showAllowed)
			for (var row = 0; row < BoardSize; row++)
				for (var col = 0; col < BoardSize; col++) {
					var cell = board1.getCell(new Location(row, col));
					if (!cell.isAssigned()) { //할당이 되어있다면
						var allowedValues = cell._allowed.allowedValuesArray(); //메모값을 가져와 출력한다.
						for (var i = 0; i < allowedValues.length; i++) {
							 val = allowedValues[i];
	
							
								var x = (col + 0.5) * CellSize+2; // center of cell for textAlign center, textBaseline middle
							...
							//console.log(val , selectValue, val == selectValue);
							context.fillStyle = (val == selectValue) ? "#e20077": normalColor;	//"#c4bcea"
		...
		for (var row = 0; row < BoardSize; row++)
			for (var col = 0; col < BoardSize; col++) {
				...
				if (value != 0) {
					if(cell.isGiven())
						context.fillStyle = "#344861";
					else if (cell.getAnswer()!=value)
	...
	function setDigitInCell(digit) {
		...
		if(memoEnable==1) //메모기능이 활성화 되어있다면
		{
		...
		{
			if (digit != 0 && cell.getAnswer()!=digit) 
			{
				missCount++;
					//alert('wrong : ' + digit + "\nans : " + cell.getAnswer() + "\nwrongcnt : " + missCount);	
			}
			...
			pushBoard(); //되돌리기 기록 
			cell.setValue(digit);
			board1.deleteMemoAll(digit,new Location(selectRow, selectCol));
		...
		updateUI();
		if ((!memoEnable==1) && (cell.getAnswer()==digit))
		{
				chkFinish();
		} 
	}
...
function init_game(){
	...
	var bo = newStartingBoard(level);	 //55
	board1.setString(printBoard(bo[1]),printBoard(bo[2])); //보드 데이터 셋팅 
...
init_game();
	...
	//스도쿠 생성 알고리즘 
	//https://ichi.pro/ko/seudoku-peojeul-saengseong-mich-haegyeol-174690016226561
