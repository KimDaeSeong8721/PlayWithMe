	...
	function relMouseCoords(event) { //마우스 이벤트 캐치 & 좌표 연산
		var totalOffsetX = 0;
		var totalOffsetY = 0;
		var canvasX = 0;
		var canvasY = 0;
	...
	AllowedValues.prototype.toggleSingle = function (n) {
	this._mask = this._mask ^ (1 << n);
	};
...
//메모해둔 값들을 어레이형태로 리턴하는 함수 Cell에 메모해둔건 이진 mask형태로 저장됨
	...
	AllowedValues.prototype.toString = function () {
		// Count number of on bits from 1..9
		var ans = '';
	...
	function Cell(value) {
		...
		this._allowed = new AllowedValues(0x0); // all possible
	...
	Cell.prototype.valueMask = function () {  //특이하게 value 마스킹을 이용함 001000000 이런식
	...
	Cell.prototype.setValue = function (n , row, col) {
		if (n < 0 || n > 9)
			throw "Illegal value not in the range 1..9.";
	...
	Cell.prototype.setMemo = function (n) {
		if (n < 0 || n > 9)
			throw "Illegal value not in the range 1..9.";
		//alert('22');
		var _allowed = this.getAllowed();
		_allowed.toggleSingle(n);
	...
	Cell.prototype.setGiven = function (n, ans) { //문제지와 정답지를 작성하기 위한 함수 
		if (n < 0 || n > 9)
			throw "Illegal value not in the range 1..9.";
		this._value = n;
		this._given = n != 0;
	...
	Cell.prototype.isGiven = function () { //문제지인가?
		return this._given;
	};

	Cell.prototype.isAssigned = function () { //할당 되었는가?
	...
	Cell.prototype.getAllowed = function (value) {
	...
	Location.prototype.getSibs = function (type) { //Cell열을 반환하는 함수 
		switch (type) {
			case SibType.Row:
				return this.rowSibs();
			case SibType.Col:
	...
	function Board() {   //보드는 어레이와 Cell로 이루어진 것임 
		...
		this._ans = MultiDimArray(BoardSize, BoardSize);
	...
	Board.prototype.deleteMemoAll= function (digit,loc) {
	//선택한 숫자의 열 행 박스의 메모를 지우도록 함 
		if (digit==0)
			return
		
		var rowSibs = loc.rowSibs();
		var colSibs = loc.colSibs();
		var squareSibs = loc.squareSibs();
		var loc;
		var cell;
		var _allowed;
		var deleteAllowed = new AllowedValues(0x0);
		deleteAllowed.setSingle(digit)
		
		//alert(rowSibs.length);
		for (var i = 0; i < rowSibs.length; i++) {
			loc = rowSibs[i];
			cell = this._digits[loc.row][loc.col];
			if (!cell.isGiven())
			{
				_allowed = cell.getAllowed(); 
				_allowed.removeValues(deleteAllowed);
			}
				
		}
		for (var i = 0; i < colSibs.length; i++) {
			loc = colSibs[i];
			cell = this._digits[loc.row][loc.col];
			...
			for (var i = 0; i < squareSibs.length; i++) {
			loc = squareSibs[i];
			cell = this._digits[loc.row][loc.col];
...
// 사용자가 값을 설정할 때마다 또는 자동 해결을 통해 호출됩니다.
// 기존 숫자를 기반으로 각 셀에 허용된 값을 업데이트합니다.
// 셀의 행, 열 또는 정사각형에 입력
		...
		// 각 셀에 대해 해당 행, col 및 square에 이미 설정된 값을 집계합니다.
		// 집계는 비트마스크이므로 이에 대한 비트 역수는 허용되는 값입니다.
		this._isValid = true;
		this._isSolved = true;
		for (var i = 0; i < locs.length; i++) {
			var loc = locs[i];
			...
			// 허용된 값을 이 행, col 또는 square에 아직 설정되지 않은 값으로 설정합니다.
	...
	*/
	...
	Board.prototype.isFinish = function () {
		
		for (var row = 0; row < BoardSize; row++)
			for (var col = 0; col < BoardSize; col++) {
				if(this._digits[row][col].getAnswer() != this._digits[row][col].getValue())
					return false;
	...
	Board.prototype.setString = function (value, ans) {
		...
		for (var row = 0; row < BoardSize; row++)
			for (var col = 0; col < BoardSize; col++) {
				...
				var ans_ch = parseInt(ans.charAt(n)); // converts '0' to 0 etc
				n++;
				var cell = this._digits[row][col];
				cell.setGiven(!isNaN(ch) ? ch : 0,!isNaN(ans_ch) ? ans_ch : 0);
			}
		//this.updateAllowed();
		return true;
	};
